...
class DFINEPostProcessor(nn.Module):
    ...
    def forward(self, outputs, orig_target_sizes: torch.Tensor):
        ...
        reids = outputs.get("pred_reid", None)
        ...
        if self.use_focal_loss:
            ...
            if reids is not None:
                reids = reids.gather(
                    dim=1, index=index.unsqueeze(-1).repeat(1, 1, reids.shape[-1])
                )
            ...
            if scores.shape[1] > self.num_top_queries:
                ...
                if reids is not None:
                    reids = torch.gather(
                        reids, dim=1, index=index.unsqueeze(-1).tile(1, 1, reids.shape[-1])
                    )
        ...
        if reids is not None:
            reids = F.normalize(reids, dim=-1)
            bytesort_parts.append(reids)
        bytesort_input = torch.cat(bytesort_parts, dim=-1)  # [B, Q, 7 + reid_dim]
        ...
        results = []
        if reids is None:
            reid_iter = [None] * boxes.shape[0]
        else:
            reid_iter = reids

        for bbox, score, lab, dep, emb, bs_input in zip(boxes, scores, labels, depths_flat, reid_iter, bytesort_input):
            result = {
                'labels': lab,
                'boxes': bbox,
                'scores': score,
