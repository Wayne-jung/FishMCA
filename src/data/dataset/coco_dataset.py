...
class CocoDetection(FasterCocoDetection, DetDataset):
    ...
    def __init__(self,
        ...
        self._track_key_to_global_id: Dict[Tuple[str, int], int] = {}
        ...
        for ds_idx, img_id in enumerate(self.ids):
            ...
            for ann in self.coco.imgToAnns.get(img_id, []):
                track_id = self._extract_track_id(ann)
                if track_id is None:
                    continue
                key = (vid, int(track_id))
                if key not in self._track_key_to_global_id:
                    self._track_key_to_global_id[key] = len(self._track_key_to_global_id)
    ...
    def load_item(self, idx):
        ...
        image_id = self.ids[idx]
        rec = self.coco.loadImgs(image_id)[0]
        image_path = os.path.join(self.img_folder, rec["file_name"])
        vid, _ = self._parse_video_and_frame(rec["file_name"])
        for obj in target:
            track_id = self._extract_track_id(obj)
            if track_id is None:
                obj["_global_track_id"] = -1
            else:
                obj["_global_track_id"] = self._track_key_to_global_id.get(
                    (vid, int(track_id)), -1
                )
    ...
    @staticmethod
    def _extract_track_id(ann):
        for key in ("track_id", "trackid", "identity_id", "instance_id", "obj_id"):
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image: Image.Image, target, **kwargs):
        ...
        track_ids = torch.tensor(
            [obj.get("_global_track_id", obj.get("track_id", -1)) for obj in anno],
            dtype=torch.int64,
        )
